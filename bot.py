...
async def help(ctx):
	...
	helpm.add_field(name = ":one: poll {question} {option 1} {optoin 2} -> Sets up a poll" , value = None , inline = False)
	helpm.add_field(name = ":two: addrole {member} {role name} -> gives the specified role to the User" , value = None , inline = False)
	helpm.add_field(name = ":three: removerole {member} {role name} -> removes the specified role from the User" , value = None , inline = False)
	...
	helpm.add_field(name = ":five: unlock -> unlocks the channel" , value = None , inline = False)
	helpm.add_field(name = ":six: server_lock {time} -> locks down the entire server for specified time" , value = "If time is not provided, it is indefinitely" , inline = False)
	...
	helpm.add_field(name = ":eight: mute {user} -> Mutes the specified user" , value = None , inline = False)
	helpm.add_field(name = ":nine: unmute {user} -> unmutes the specified user" , value = None , inline = False)
	helpm.add_field(name = ":keycap_ten: kick {user} -> kicks the specified user" , value = None , inline = False)
	helpm.add_field(name = ":one::one: ban {user} -> bans the specified user" , value = None , inline = False)
	helpm.add_field(name = ":one::two: unban {user} -> unbans the specified user" , value = None , inline = False)
	helpm.add_field(name = ":one::three: pop -> creates a bubblewrap of spoilers" , value = None , inline = False)
	helpm.add_field(name = ":one::four: timer {time} -> sets a timer" , value = "hour -> hr , minutes -> m, seconds -> s" , inline = False)
	helpm.add_field(name = ":one::five: ping -> Shows the bot's ping" , value = None , inline = False)
	helpm.add_field(name = ":one::six: clear {amount} -> Deletes the specified amount of messages" , value = None , inline = False)
	helpm.add_field(name = ":one::seven: rainbow {role} -> creates a rainbow effect for that role" , value = None , inline = False)
	helpm.add_field(name = ":one::eight: 8ball {question} -> gives a random answer to your question" , value = None , inline = False)
